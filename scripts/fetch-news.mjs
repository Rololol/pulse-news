...
const topicOf=(t,s)=>{const title=t.toLowerCase(),body=s.toLowerCase();const rules=[["Politik",/politik|politic|government|election|wahl|parliament|minister|president|congress|senate|chancellor|kanzler|european commission|trump|prime minister|presidential/],["Technologie",/technolog|technology|software|chip|artificial intelligence|\bai\b|ki |digital/],["Wissenschaft",/wissenschaft|science|research|study|laboratory|physic|biology/],["Gesundheit",/gesund|health|medizin|medical|hospital|disease|cancer/],["Finanzen",/wirtschaft|business|econom|market|markets|finance|financial|bank|company|corporate|trade/],["Klima",/klima|climate|weather|energie|energy|emission|carbon|environment|wildfire|flood|storm/],["Sport",/sport|football|soccer|tennis|basketball|olympic/],["Kultur",/kultur|culture|film|music|art|book/],["Mobilität",/mobil|transport|car|auto|rail|aviation/],["Bildung",/bildung|education|school|university/],["Raumfahrt",/raumfahrt|space|nasa|astronom|planet|moon|mars/]];for(const [name,re] of rules)if(re.test(title))return name;for(const [name,re] of rules)if(re.test(body))return name;return"Alle"};
